...
export default function ReviewQueue() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-stone-900">Review Queue</h2>
        <p className="mt-1 text-sm text-stone-500">默认自动通过，但 fatal scene 会进入这里等待处理。</p>
      </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-stone-900">
                  Run #{item.run_id} / 第 {item.chapter_number} 章 / Scene {item.scene_number}
                </p>
                <p className="mt-1 text-xs text-stone-500">{item.reason || '无说明'}</p>
              </div>
              <span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs text-amber-700">{item.status}</span>
            </div>
            <textarea
              value={patchMap[item.id] ?? item.patch_text}
              onChange={(event) => setPatchMap((prev) => ({ ...prev, [item.id]: event.target.value }))}
              placeholder="需要的话可以直接提交 patch 文本。"
