...
> **部署后仅需以下命令，即可在浏览器中完成所有配置与操作。**

```bash
# 1. 复制并编辑环境变量（填写安全密钥与管理员账号）
cp .env.example .env

# 2. 一键启动（首次构建约需 2~5 分钟）
docker compose up -d --build
# 或使用脚本：bash scripts/up.sh

# 3. 打开浏览器
# Web 控制台：http://127.0.0.1:18080
# 后端 API  ：http://127.0.0.1:18000（调试用）
```

启动后在浏览器中完成：
1. 登录（默认账号 `admin`，密码见 `.env` 中的 `POIESIS_ADMIN_PASS`）
2. 进入 **系统设置** → 配置 OpenAI/Anthropic API Key
3. 进入 **系统设置** → 点击 **初始化世界**
4. 进入 **运行控制** → 设置章节数 → 点击 **开始生成**
